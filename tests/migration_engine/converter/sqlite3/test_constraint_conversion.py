...
def test_primary_key_constraint_conversion():
    ...
    assert SQLite3Converter.convert_constraint_to_SQL(constraint) == "PRIMARY KEY"
...
def test_unique_constraint_conversion():
    ...
    assert SQLite3Converter.convert_constraint_to_SQL(constraint) == "UNIQUE"
...
def test_default_constraint_conversion():
    ...
    with raises(UnsupportedConstraintError):
        SQLite3Converter.convert_constraint_to_SQL(
...
def test_single_column_foreign_key_constraint_conversion():
    ...
    assert SQLite3Converter.convert_constraint_to_SQL(constraint) == (
...
def test_check_constraint_conversion():
    ...
    assert SQLite3Converter.convert_constraint_to_SQL(constraint) == "CHECK (age > 18)"
